...
@Composable
fun HistoryScreen(chViewModel: ModelData, ebsDeviceMonitor: EBSDeviceMonitor, navController: NavController, updateHideBottomBar: (Boolean) -> Unit) {
    Scaffold(
        ...
        val sweatList = listOf(stringResource(R.string.title_water), stringResource(R.string.title_sodium))
        val timeList = listOf(stringResource(R.string.history_day), stringResource(R.string.week), stringResource(R.string.history_month))
                    ...
                    Column {
                        ...
                        Column {
                            if (timeChange.value == 0) {
                                    ...
                                    update = { lineChart ->
                                        ...
                                        if (sweatElectrolyteDisplaySelection.value == 0) {
                                            ...
                                            lineChartEntrySweatSamples =
                                                sweatHistoricalData.map { historicalSweatData ->
                                                    Entry(((epochTime + historicalSweatData.timeStamp).toFloat()/* / 3600.0f*/), chViewModel.userPrefsData.handleUserSweatConversionOz(historicalSweatData.sweatVolumeDeficitInOz * -1.0).toFloat())
                                                }

                                            lossDataSet = LineDataSet(lineChartEntrySweatSamples, chViewModel.userPrefsData.getUserSweatUnitString()).apply {
                                            ...
                                            lineChartEntrySweatSamples =
                                                sweatHistoricalData.map { historicalSweatData ->
                                                    Entry(((epochTime + historicalSweatData.timeStamp).toFloat()/* / 3600.0f*/), ((historicalSweatData.sweatSodiumDeficitInMg * -1.0).toFloat()))
                                                }

                                            lossDataSet = LineDataSet(lineChartEntrySweatSamples, chViewModel.userPrefsData.getUserSodiumUnitString()).apply {
                                        ...
                                        lineChart.apply {
                                            ...
                                            xAxis.apply {
                                                ...
                                                axisMaximum = if (sweatHistoricalData.size > 0) ((epochTime + sweatHistoricalData.last().timeStamp).toFloat()/* / 3600.0f*/ + 1800.0f) else 1.0f
                                            ...
                                            axisLeft.apply {
                                                ...
                                                if (sweatElectrolyteDisplaySelection.value == 0) {
                                                    ...
                                                    if (chViewModel.userPrefsData.getUnits().value == 1) {
                                                        if(lineX.yMax > 35.0) {
                                                            axisMaximum = (((lineX.yMax / 10.0).roundToInt() + 2.0) * 10.0).toFloat()
                                                            axisMinimum = -1.0F * axisMaximum
                                                        }
                                                        if(lineX.yMin < -35.0) {
                                                            axisMinimum = (((lineX.yMin / 10.0).roundToInt() - 2.0) * 10.0).toFloat()
                                                            axisMaximum = -1.0F * axisMinimum
                                                        }
                                                    }
                                                    else {
                                                        if(lineX.yMax > 875.0) {
                                                            axisMaximum = (((lineX.yMax / 250.0).roundToInt() + 2.0) * 250.0).toFloat()
                                                            axisMinimum = -1.0F * axisMaximum

                                                        }
                                                        if(lineX.yMin < -875.0) {
                                                            axisMinimum = (((lineX.yMin / 250.0).roundToInt() - 2.0) * 250.0).toFloat()
                                                            axisMaximum = -1.0F * axisMinimum
                                                        }
                                                    }
                                                    ...
                                                    axisMaximum = 1000.0f
                                                    if(lineX.yMax > 800.0) {
                                                        axisMaximum = (((lineX.yMax / 100.0).roundToInt() + 2.0) * 100.0).toFloat()
                                                        axisMinimum = -1.0F * axisMaximum
                                                    }
                                                    if(lineX.yMin < -800.0) {
                                                        axisMinimum = (((lineX.yMin / 100.0).roundToInt() - 2.0) * 100.0).toFloat()
                                                        axisMaximum = -1.0F * axisMinimum
                                                    }
                                ...
                                HistoricalBarChart(chViewModel = chViewModel, ebsDeviceMonitor, sweatDisplayChange = sweatElectrolyteDisplaySelection.value, timeChange = timeChange.value)
                            }
                        }
                        ...
                        Column(
                            ...
                            if (timeChange.value == 0) {
                                Text(stringResource(R.string.work_time_hr),
                                    Modifier.padding(start = 20.dp, top = 10.dp, bottom = 20.dp),
                                    fontFamily = OswaldFonts,
                                    fontSize = 12.sp,
